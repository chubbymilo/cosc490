...
va1=$(docker exec admiring_bassi /bin/sh -c "grep -n 'eth0:' $network_file")
target_index=${va1:0:1}

nc -l $port | tar -x &
export pid5=$!

docker exec admiring_bassi /bin/sh -c "/tmp/exec/test_gather_data5 $cpu_file $cpu_out_file $repeat $time_interval & export pid1=$! ;
...
va1=$(docker exec admiring_bassi /bin/sh -c "grep -n 'eth0:' $network_file2")
...
docker exec admiring_bassi /bin/sh -c "/tmp/exec/test_gather_data5 $cpu_file $cpu_out_file $repeat $time_interval & export pid1=$! ;
/tmp/exec/test_gather_data5 $cpu_file2 $cpu_out_file2 $repeat $time_interval & export pid6=$! ;
/tmp/exec/test_gather_data5 $memory_file $memory_file2 $memory_out_file $repeat $time_interval & export pid2=$! ;
/tmp/exec/test_gather_data5 $memory_file3 $memory_file4 $memory_out_file2 $repeat $time_interval & export pid7=$! ;
/tmp/exec/test_gather_data5 $block_file $block_out_file $repeat $time_interval & export pid3=$! ;
/tmp/exec/test_gather_data5 $block_file2 $block_out_file2 $repeat $time_interval & export pid8=$! ;
/tmp/exec/test_gather_data5 $network_file $network_out_file $target_index $repeat $time_interval & export pid4=$! ;
/tmp/exec/test_gather_data5 $network_file2 $network_out_file2 $target_index2 $repeat $time_interval & export pid9=$! &&
...
rm $cpu_out_file ; rm $memory_out_file ; rm $block_out_file ; rm $network_out_file ;
rm $cpu_out_file2 ; rm $memory_out_file2 ; rm $block_out_file2 ; rm $network_out_file2 " & python3 process_data3.py $1 $2 $3 
